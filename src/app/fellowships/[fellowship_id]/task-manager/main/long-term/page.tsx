...
interface ITask {
  ...
  metadata: Record<string, unknown>; // Corrected: Using unknown instead of any
  tags: string[];
...
export default function AssignmentsPage() {
  const { tasks, teamMembers, assignTask } = useStore() as unknown as {
    tasks: ITask[];
  ...
  const unassignedTasks: ITask[] = tasks.filter((task: ITask) => !task.assignee_id)
  const selectedMemberData: ITeamMember | undefined = teamMembers.find((member: ITeamMember) => member.id === selectedMember)
  const memberTasks: ITask[] = selectedMember ? tasks.filter((task: ITask) => task.assignee_id === selectedMember) : []
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {selectedMemberData && (
            <Card className="bg-gray-800 border-gray-700">
              <CardHeader>
                {/* Corrected: Escaping the apostrophe */}
