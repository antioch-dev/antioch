...
export default function RecurringTasksPage() {
  ...
  const handleSubmit = () => {
    ...
    const taskData = {
      ...formData,
      isActive: true,
      // Fix: Use nullish coalescing to provide a fallback date.
      nextOccurrence: new Date(formData.startDate ?? new Date()),
  ...
  const handleEdit = (task: RecurringTask) => {
    // Fix: Safely handle both Date and string types for startDate.
    const startDate = task.startDate instanceof Date ? task.startDate.toISOString().split("T")[0] : task.startDate ?? new Date().toISOString().split("T")[0];
  ...
  const getNextOccurrenceText = (task: RecurringTask) => {
    // Fix: Use nullish coalescing for a safe fallback.
    const nextDate = task.nextOccurrence instanceof Date ? task.nextOccurrence : new Date(task.nextOccurrence ?? new Date());
  ...
  const getStartDateText = (task: RecurringTask) => {
    // Fix: Use nullish coalescing for a safe fallback.
    const startDate = task.startDate instanceof Date ? task.startDate : new Date(task.startDate ?? new Date());
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value: RecurringTask["frequency"]) => setFormData({ ...formData, frequency: value })}
                  >
