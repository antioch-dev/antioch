...
export default function RecurringTasksPage() {
  ...
  const handleEdit = (task: RecurringTask) => {
    ...
    const startDate = task.startDate instanceof Date ? task.startDate : new Date(task.startDate as string)
    setFormData({
      title: task.title,
  ...
  const handleToggle = (id: string) => {
    ...
    const task = recurringTasks.find((t:any) => t.id === id)
    toast.success(`Task ${task?.isActive ? "paused" : "activated"}!`)
  }

  const getNextOccurrenceText = (task: RecurringTask) => {
    const nextDate = task.nextOccurrence instanceof Date ? task.nextOccurrence : new Date(task.nextOccurrence as string)
  ...
  const activeCount = recurringTasks.filter((task:any) => task.isActive).length
  const inactiveCount = recurringTasks.filter((task:any) => !task.isActive).length
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-400">
                  {
                    recurringTasks.filter((task:any) => {
                      // Ensure task.nextOccurrence is treated as a string before passing to new Date()
                      const nextDate =
                        task.nextOccurrence instanceof Date ? task.nextOccurrence : new Date(task.nextOccurrence as string)
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-gray-300 text-sm">{frequencyLabels[task.frequency as keyof typeof frequencyLabels]}</span>
                  </div>
                  <div className="flex items-center space-x-2">
