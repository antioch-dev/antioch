...
import { mockPrayerAssignments, mockPrayerRequests, getPrayerRequestById, type PrayerAssignment } from "@/lib/mock-data"
import {
  Plus,
  UserCheck,
  Clock,
...
interface PrayerAssignmentsProps {
  params: {
    fellowship: string
  }
}

export default function PrayerAssignments({ params }: PrayerAssignmentsProps) {
  const { fellowship } = params
  const [assignments, setAssignments] = useState<PrayerAssignment[]>(mockPrayerAssignments)
  ...
  // Form state for completing assignments
  ...
  const unassignedRequests = mockPrayerRequests.filter(
    (request) => !assignments.some((assignment) => assignment.requestId === request.id),
  )
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 text-sm text-gray-600">
                            <div className="flex items-center gap-1">
                              ...
                              Assigned to {assignment.assignedMember}
                            </div>
                            <div className="flex items-center gap-1">
                              <Calendar className="h-4 w-4" />
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 text-sm text-gray-600">
                            <div className="flex items-center gap-1">
                              <User className="h-4 w-4" />
                              Completed by {assignment.assignedMember}
                            </div>
                            <div className="flex items-center gap-1">
                              <Calendar className="h-4 w-4" />
                              {assignment.dateCompleted && new Date(assignment.dateCompleted).toLocaleDateString()}
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
