...
export class ApiClient {
  ...
  async getProxies(filters?: ProxyFilters): Promise<ApiResponse<StreamingProxy[]>> {
    ...
    const endpoint = `/fellowship1/streaming-proxies${queryString ? `?${queryString}` : ''}`;
  ...
  async getProxy(id: string): Promise<ApiResponse<StreamingProxy>> {
    return this.get<ApiResponse<StreamingProxy>>(`/fellowship1/streaming-proxies/${id}`);
  }
  ...
  async createProxy(data: CreateProxyRequest): Promise<ApiResponse<StreamingProxy>> {
    return this.post<ApiResponse<StreamingProxy>, CreateProxyRequest>('/fellowship1/streaming-proxies', data);
  }
  ...
  async updateProxy(id: string, data: UpdateProxyRequest): Promise<ApiResponse<StreamingProxy>> {
    return this.patch<ApiResponse<StreamingProxy>>(`/fellowship1/streaming-proxies/${id}`, data as { error?: string; message?: string; details?: Record<string, string[]> });
  }
  ...
  async deleteProxy(id: string): Promise<ApiResponse<void>> {
    return this.delete<ApiResponse<void>>(`/fellowship1/streaming-proxies/${id}`);
  }
  ...
  async runHealthCheck(id: string): Promise<ApiResponse<HealthCheckResult>> {
    return this.post<ApiResponse<HealthCheckResult>>(`/fellowship1/streaming-proxies/${id}/health-check`);
  }
  ...
  async runAllHealthChecks(): Promise<ApiResponse<HealthCheckResult[]>> {
    return this.post<ApiResponse<HealthCheckResult[]>>('/fellowship1/streaming-proxies/health-check');
  }
  ...
  async getSystemStats(): Promise<ApiResponse<SystemStats>> {
    return this.get<ApiResponse<SystemStats>>('/fellowship1/streaming-proxies/stats');
  }
  ...
  async getAnalytics(timeRange: '7d' | '30d' | '90d' = '7d'): Promise<ApiResponse<AnalyticsData>> {
    return this.get<ApiResponse<AnalyticsData>>(`/fellowship1/streaming-proxies/analytics?timeRange=${timeRange}`);
  }
  ...
  async getActiveSessions(proxyId: string): Promise<ApiResponse<StreamingSession[]>> {
    return this.get<ApiResponse<StreamingSession[]>>(`/fellowship1/streaming-proxies/${proxyId}/sessions/active`);
  }
}
