...
export class WebSocketManager {
  ...
  private setupEventListeners(): void {
    ...
    this.ws.onmessage = (event: MessageEvent) => {
      try {
        const data = typeof event.data === 'string' ? JSON.parse(event.data) as RealTimeUpdate : null;

        // Handle heartbeat/ping messages
        if (data!.type === 'ping' as unknown as string) {
  ...
  private startHeartbeat(): void {
    ...
    this.heartbeatIntervalId = setInterval(() => {
      if (this.ws?.readyState === WebSocket.OPEN) {
        this.send({ type: 'ping' } as unknown as string);
      }
...
private isValidRealTimeUpdate(data: unknown): data is RealTimeUpdate {
  if (typeof data !== 'object' || data === null) return false;

  // Safely treat as a dictionary of unknowns
