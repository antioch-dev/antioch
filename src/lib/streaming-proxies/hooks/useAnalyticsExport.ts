...
export function useAnalyticsExport(
  ...
  // Removed duplicate declaration of deleteJob
  ...
  const startExport = useCallback(async (options: ExportOptions): Promise<string> => {
    ...
    try {
      ...
      const result = await response.json() as {
        success: boolean;
        downloadUrl?: string;
        data?: null; // For JSON and CSV exports
        error?: string;
        metadata?: {
          fileSize?: string;
        };
  ...
  }, [jobs, maxConcurrentExports, generateJobName, clearError, updateJob, autoCleanupCompleted, cleanupDelay, handleError]);
