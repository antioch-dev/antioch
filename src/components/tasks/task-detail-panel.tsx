...
import { useStore, type Task, type TaskStatus, type TaskPriority } from "@/lib/store"
import { cn } from "@/lib/utils"
import { useState, useEffect } from "react" // Added useEffect for synchronization
...
export function TaskDetailPanel() {
  ...
  const [editedTask, setEditedTask] = useState<Task | null>(selectedTask) // Added a useEffect to synchronize editedTask with selectedTask
  // whenever the selectedTask changes. This is important when a new
  // task is selected while the panel is already open.
  ...
  const handleSave = () => {
    if (editedTask) {
      ...
      updateTask(selectedTask.id, editedTask) // Update the locally selected task to the new version
  ...
  const handleCancel = () => {
    // Reset the edited task to the original selected task
    setEditedTask(selectedTask)
    setIsEditing(false)
  }
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                {isEditing ? (
                  ...
                  <Badge className={cn("text-white border-0 capitalize", statusColors[selectedTask.status])}>
